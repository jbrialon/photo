...
export default {
  ...
  computed: {
    photos () {
      ...
      const photos = req.keys()
        ...
        .map(item => req(item))
        .map((item) => {
          const parsedDate = parse(item.exif.CreateDate, 'y:MM:dd HH:mm:ss', new Date(), { locale: enGB })
          item.date = {
  ...
  methods: {
    animateLine (timestamp) {
      ...
      // stop if it finishes
      if (this.animationParams.index >= geojson.features[0].geometry.coordinates.length) {
        let lastPos = [
          geojson.features[0].geometry.coordinates[geojson.features[0].geometry.coordinates.length - 1][0],
          geojson.features[0].geometry.coordinates[geojson.features[0].geometry.coordinates.length - 1][1]
        ]
        ...
        let pos = [
          geojson.features[0].geometry.coordinates[this.animationParams.index][0],
          geojson.features[0].geometry.coordinates[this.animationParams.index][1]
        ]
        ...
        const photo = find(this.photos, (photo) => {
          return Number(photo.date.day) === currentDay && Number(photo.date.hour) === currentDate.getHours() && inRange(currentDate.getMinutes(), Number(photo.date.minute) - 9, Number(photo.date.minute) + 9)
        })
